...
def main():
    ...
    # 3) Set up the single-table SQLite database
    conn, cursor = setup_database("leaves.db")
    
    # 4) Index the downloaded books (folder: "Gutenberg_Top_100")
    folder = "Gutenberg_Top_100"
    occurrences_map = index_books(folder, suffix_to_id)
    
    # 5) Save the occurrences (doc->offset mappings) in the 'leaves' table
    store_occurrences(cursor, occurrences_map)
    conn.commit()
    
    # 6) Search loop: repeatedly prompt the user for a suffix to search
    while True:
